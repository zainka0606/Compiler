...
AST ParseProgram(std::string_view source_text) {
    try {
        ...
        throw ParserException("internal grammar parse error at " + std::to_string(ex.line()) + ":" +
                              std::to_string(ex.column()) + ": " + ex.what());
    } catch (const compiler::parsergen::BuildException& ex) {
        throw ParserException(std::string("internal parser generator build error: ") + ex.what());
