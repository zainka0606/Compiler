...
using SpecParser = generated::ParserGeneratorStage2Spec::ParserGeneratorStage2SpecParser;
...
std::vector<GenericToken> ToGenericTokens(const std::vector<SpecToken>& tokens) {
...
bool MatchesReduction(const LR1ParseTable& parse_table,
                      const CSTNode& node,
                      std::string_view lhs,
                      std::initializer_list<std::string_view> rhs) {
    return compiler::parsergen::CSTNodeMatchesProduction(parse_table, node, lhs, rhs);
}

std::string ExpectTerminalLexeme(const CSTNode& node) {
    if (!node.IsTerminal()) {
        throw BuildException("expected terminal CST node, got '" + std::string(node.Symbol()) + "'");
    }
...
ASTNodeTypeDecl::FieldDecl ParseFieldSpec(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "FieldSpec") {
        throw BuildException("expected FieldSpec node, got '" + std::string(node.Symbol()) + "'");
    }
...
std::vector<ASTNodeTypeDecl::FieldDecl> ParseFieldSpecList(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "FieldSpecList") {
        throw BuildException("expected FieldSpecList node, got '" + std::string(node.Symbol()) + "'");
    }

    if (MatchesReduction(parse_table, node, "FieldSpecList", {"FieldSpec"})) {
        return {ParseFieldSpec(node.Child(0), parse_table)};
    }
    if (MatchesReduction(parse_table, node, "FieldSpecList", {"FieldSpecList", "COMMA", "FieldSpec"})) {
        std::vector<ASTNodeTypeDecl::FieldDecl> out = ParseFieldSpecList(node.Child(0), parse_table);
...
ASTNodeTypeDecl::VirtualMethodDecl ParseVirtualMethodDecl(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "VirtualMethodDecl") {
        throw BuildException("expected VirtualMethodDecl node, got '" + std::string(node.Symbol()) + "'");
    }
    if (!MatchesReduction(parse_table, node, "VirtualMethodDecl",
                          {"KW_VIRTUAL", "IDENT", "LPAREN", "RPAREN", "ARROW", "IDENT", "SEMI"})) {
...
void AppendVirtualMethodDeclList(const CSTNode& node,
                                 const LR1ParseTable& parse_table,
                                 std::vector<ASTNodeTypeDecl::VirtualMethodDecl>& out) {
    if (node.Symbol() != "VirtualMethodDeclList") {
        throw BuildException("expected VirtualMethodDeclList node, got '" + std::string(node.Symbol()) + "'");
    }

    if (MatchesReduction(parse_table, node, "VirtualMethodDeclList", {"VirtualMethodDecl"})) {
        out.push_back(ParseVirtualMethodDecl(node.Child(0), parse_table));
        return;
    }
    if (MatchesReduction(parse_table, node, "VirtualMethodDeclList", {"VirtualMethodDeclList", "VirtualMethodDecl"})) {
...
ASTNodeTypeDecl::VirtualMethodImplDecl ParseMethodImplDecl(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "MethodImplDecl") {
        throw BuildException("expected MethodImplDecl node, got '" + std::string(node.Symbol()) + "'");
    }
    if (!MatchesReduction(parse_table, node, "MethodImplDecl",
                          {"KW_OVERRIDE", "IDENT", "LPAREN", "RPAREN", "ARROW", "IDENT", "EQUAL", "CODE", "SEMI"})) {
...
void AppendMethodImplDeclList(const CSTNode& node,
                              const LR1ParseTable& parse_table,
                              std::vector<ASTNodeTypeDecl::VirtualMethodImplDecl>& out) {
    if (node.Symbol() != "MethodImplDeclList") {
        throw BuildException("expected MethodImplDeclList node, got '" + std::string(node.Symbol()) + "'");
    }

    if (MatchesReduction(parse_table, node, "MethodImplDeclList", {"MethodImplDecl"})) {
        out.push_back(ParseMethodImplDecl(node.Child(0), parse_table));
        return;
    }
    if (MatchesReduction(parse_table, node, "MethodImplDeclList", {"MethodImplDeclList", "MethodImplDecl"})) {
...
void FinalizeAstDeclFields(ASTNodeTypeDecl& decl, std::vector<ASTNodeTypeDecl::FieldDecl> field_decls) {
...
std::vector<std::string> ParseSymbolList(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "SymbolList") {
        throw BuildException("expected SymbolList node, got '" + std::string(node.Symbol()) + "'");
    }

    if (MatchesReduction(parse_table, node, "SymbolList", {"IDENT"})) {
        return {ExpectTerminalLexeme(node.Child(0))};
    }
    if (MatchesReduction(parse_table, node, "SymbolList", {"SymbolList", "IDENT"})) {
        std::vector<std::string> out = ParseSymbolList(node.Child(0), parse_table);
...
ActionArg ParseActionArg(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "ActionArg") {
        throw BuildException("expected ActionArg node, got '" + std::string(node.Symbol()) + "'");
    }

    if (MatchesReduction(parse_table, node, "ActionArg", {"DOLLAR", "INT"})) {
        return ActionArg{.kind = ActionArgKind::ChildAST, .rhs_index = ParseOneBasedIndex(node.Child(1))};
    }
    if (MatchesReduction(parse_table, node, "ActionArg", {"DOLLAR", "INT", "DOT", "KW_LEXEME"})) {
        return ActionArg{.kind = ActionArgKind::ChildLexeme, .rhs_index = ParseOneBasedIndex(node.Child(1))};
    }

    throw BuildException("unsupported ActionArg CST shape");
}

std::vector<ActionArg> ParseActionArgList(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "ActionArgList") {
        throw BuildException("expected ActionArgList node, got '" + std::string(node.Symbol()) + "'");
    }

    if (MatchesReduction(parse_table, node, "ActionArgList", {"ActionArg"})) {
        return {ParseActionArg(node.Child(0), parse_table)};
    }
    if (MatchesReduction(parse_table, node, "ActionArgList", {"ActionArgList", "COMMA", "ActionArg"})) {
        std::vector<ActionArg> out = ParseActionArgList(node.Child(0), parse_table);
...
RuleAction ParseActionExpr(const CSTNode& node, const LR1ParseTable& parse_table) {
    if (node.Symbol() != "ActionExpr") {
        throw BuildException("expected ActionExpr node, got '" + std::string(node.Symbol()) + "'");
    }
    ...
    if (callee == "Pass") {
        if (args.size() != 1 || args[0].kind != ActionArgKind::ChildAST) {
            throw BuildException("Pass(...) requires exactly one child AST argument like Pass($1)");
        }
        return RuleAction{
...
RuleAlternative ParseRuleAlt(const CSTNode& node, const LR1ParseTable& parse_table) {
    ...
    if (MatchesReduction(parse_table, node, "RuleAlt", {"SymbolList", "FATARROW", "ActionExpr"})) {
        return RuleAlternative{
            .symbols = ParseSymbolList(node.Child(0), parse_table),
...
void AppendRuleAltList(const CSTNode& node,
                       const LR1ParseTable& parse_table,
                       std::vector<RuleAlternative>& out) {
    if (node.Symbol() != "RuleAltList") {
        throw BuildException("expected RuleAltList node, got '" + std::string(node.Symbol()) + "'");
    }
    ...
    if (MatchesReduction(parse_table, node, "RuleAltList", {"RuleAltList", "PIPE", "RuleAlt"})) {
...
void ParseDecl(const CSTNode& node, const LR1ParseTable& parse_table, Stage2SpecAST& spec);

void AppendDeclList(const CSTNode& node, const LR1ParseTable& parse_table, Stage2SpecAST& spec) {
    if (node.Symbol() != "DeclList") {
        throw BuildException("expected DeclList node, got '" + std::string(node.Symbol()) + "'");
    }
...
void ParseDecl(const CSTNode& node, const LR1ParseTable& parse_table, Stage2SpecAST& spec) {
    if (node.Symbol() != "Decl") {
        throw BuildException("expected Decl node, got '" + std::string(node.Symbol()) + "'");
    }
    ...
    if (MatchesReduction(parse_table, node, "Decl", {"AstBaseDecl"})) {
        ...
        if (MatchesReduction(parse_table, a, "AstBaseDecl", {"KW_ASTBASE", "IDENT", "SEMI"})) {
            // no base, no methods
        } else if (MatchesReduction(parse_table, a, "AstBaseDecl", {"KW_ASTBASE", "IDENT", "COLON", "IDENT", "SEMI"})) {
            ...
            AppendVirtualMethodDeclList(a.Child(3), parse_table, decl.virtual_methods);
        } else if (MatchesReduction(parse_table, a, "AstBaseDecl",
                                    {"KW_ASTBASE", "IDENT", "COLON", "IDENT", "LBRACE", "VirtualMethodDeclList", "RBRACE", "SEMI"})) {
            decl.base_type_name = ExpectTerminalLexeme(a.Child(3));
            AppendVirtualMethodDeclList(a.Child(5), parse_table, decl.virtual_methods);
        } else {
    ...
    if (MatchesReduction(parse_table, node, "Decl", {"AstDecl"})) {
        ...
        if (MatchesReduction(parse_table, a, "AstDecl", {"KW_AST", "IDENT", "LPAREN", "RPAREN", "SEMI"})) {
            FinalizeAstDeclFields(decl, {});
            ...
            FinalizeAstDeclFields(decl, ParseFieldSpecList(a.Child(3), parse_table));
        } else if (MatchesReduction(parse_table, a, "AstDecl",
                                    {"KW_AST", "IDENT", "COLON", "IDENT", "LPAREN", "FieldSpecList", "RPAREN", "SEMI"})) {
            decl.base_type_name = ExpectTerminalLexeme(a.Child(3));
            FinalizeAstDeclFields(decl, ParseFieldSpecList(a.Child(5), parse_table));
        } else if (MatchesReduction(parse_table, a, "AstDecl",
                                    {"KW_AST", "IDENT", "LPAREN", "RPAREN", "LBRACE", "MethodImplDeclList", "RBRACE", "SEMI"})) {
            FinalizeAstDeclFields(decl, {});
            AppendMethodImplDeclList(a.Child(5), parse_table, decl.virtual_method_impls);
        } else if (MatchesReduction(parse_table, a, "AstDecl",
                                    {"KW_AST", "IDENT", "COLON", "IDENT", "LPAREN", "RPAREN", "LBRACE", "MethodImplDeclList", "RBRACE", "SEMI"})) {
            decl.base_type_name = ExpectTerminalLexeme(a.Child(3));
            FinalizeAstDeclFields(decl, {});
            AppendMethodImplDeclList(a.Child(7), parse_table, decl.virtual_method_impls);
        } else if (MatchesReduction(parse_table, a, "AstDecl",
                                    {"KW_AST", "IDENT", "LPAREN", "FieldSpecList", "RPAREN", "LBRACE", "MethodImplDeclList", "RBRACE", "SEMI"})) {
            FinalizeAstDeclFields(decl, ParseFieldSpecList(a.Child(3), parse_table));
            AppendMethodImplDeclList(a.Child(6), parse_table, decl.virtual_method_impls);
        } else if (MatchesReduction(parse_table, a, "AstDecl",
                                    {"KW_AST", "IDENT", "COLON", "IDENT", "LPAREN", "FieldSpecList", "RPAREN", "LBRACE",
                                     "MethodImplDeclList", "RBRACE", "SEMI"})) {
            decl.base_type_name = ExpectTerminalLexeme(a.Child(3));
            FinalizeAstDeclFields(decl, ParseFieldSpecList(a.Child(5), parse_table));
            AppendMethodImplDeclList(a.Child(8), parse_table, decl.virtual_method_impls);
        } else {
...
ValidationContext BuildValidationContext(const Stage2SpecAST& spec) {
    ...
    for (const RuleDefinition& rule : spec.rules) {
        if (!ctx.nonterminal_set.insert(rule.lhs).second) {
            throw BuildException("duplicate rule declaration for nonterminal: " + rule.lhs);
        }
    }
    for (const ASTNodeTypeDecl& node : spec.ast_node_types) {
        if (ctx.ast_types_by_name.find(node.name) != ctx.ast_types_by_name.end()) {
            throw BuildException("duplicate AST node type declaration: " + node.name);
        }
        ...
        for (const ASTNodeTypeDecl::FieldDecl& field : node.field_decls) {
            if (!field_names.insert(field.name).second) {
                throw BuildException("duplicate field '" + field.name + "' in AST node type '" + node.name + "'");
            }
        }
        std::unordered_set<std::string> method_names;
        for (const ASTNodeTypeDecl::VirtualMethodDecl& method : node.virtual_methods) {
            if (!method_names.insert(method.name).second) {
                throw BuildException("duplicate virtual method '" + method.name + "' in AST type '" + node.name + "'");
            }
        }
        std::unordered_set<std::string> method_impl_names;
        for (const ASTNodeTypeDecl::VirtualMethodImplDecl& impl : node.virtual_method_impls) {
            if (!method_impl_names.insert(impl.name).second) {
                throw BuildException("duplicate virtual method implementation '" + impl.name + "' in AST type '" +
                                     node.name + "'");
            }
        }
        if (node.is_abstract && !node.field_decls.empty()) {
            throw BuildException("astbase '" + node.name + "' cannot declare fields; use ast for concrete node types");
        }
    }

    for (const std::string& terminal : ctx.terminal_set) {
        if (ctx.nonterminal_set.find(terminal) != ctx.nonterminal_set.end()) {
            throw BuildException("symbol declared as both token and rule: " + terminal);
        }
    }
    ...
    for (const ASTNodeTypeDecl& node : spec.ast_node_types) {
        if (!node.base_type_name.empty()) {
            ...
            if (base_it == ctx.ast_types_by_name.end()) {
                throw BuildException("unknown AST base type '" + node.base_type_name + "' for '" + node.name + "'");
            }
            if (!base_it->second->is_abstract) {
                throw BuildException("AST type '" + node.name + "' must inherit from an astbase, but '" +
                                     node.base_type_name + "' is not declared with astbase");
            }
        }
        ...
        for (const ASTNodeTypeDecl::FieldDecl& field : node.field_decls) {
            ...
            if (ctx.ast_types_by_name.find(field.type_name) == ctx.ast_types_by_name.end()) {
                throw BuildException("unknown AST field type '" + field.type_name + "' for field '" + field.name +
...
void CollectInheritedVirtualMethods(const ASTNodeTypeDecl& type_decl,
                                    const ValidationContext& ctx,
                                    std::unordered_map<std::string, std::string>& methods_out,
                                    std::unordered_set<std::string>& visiting) {
    if (!visiting.insert(type_decl.name).second) {
        throw BuildException("cyclic AST base inheritance involving '" + type_decl.name + "'");
    }

    if (!type_decl.base_type_name.empty()) {
        const auto base_it = ctx.ast_types_by_name.find(type_decl.base_type_name);
        if (base_it == ctx.ast_types_by_name.end()) {
            throw BuildException("unknown AST base type '" + type_decl.base_type_name + "' for '" + type_decl.name +
                                 "'");
        }
        CollectInheritedVirtualMethods(*base_it->second, ctx, methods_out, visiting);
    }

    for (const ASTNodeTypeDecl::VirtualMethodDecl& method : type_decl.virtual_methods) {
        const auto found = methods_out.find(method.name);
        if (found != methods_out.end() && found->second != method.return_type_name) {
            throw BuildException("virtual method '" + method.name + "' in AST type '" + type_decl.name +
                                 "' changes return type from '" + found->second + "' to '" + method.return_type_name +
                                 "'");
        }
...
void ValidateMethodImplementations(const ASTNodeTypeDecl& node, const ValidationContext& ctx) {
    if (node.virtual_method_impls.empty()) {
        return;
    }
    if (node.is_abstract) {
        throw BuildException("astbase '" + node.name + "' cannot contain override method implementations");
    }
    if (node.base_type_name.empty()) {
        throw BuildException("AST node type '" + node.name +
                             "' declares override methods but has no astbase parent to override");
    }
    ...
    CollectInheritedVirtualMethods(*base_it->second, ctx, inherited_virtual_methods, visiting);

    std::unordered_set<std::string> seen_impl_names;
    for (const ASTNodeTypeDecl::VirtualMethodImplDecl& impl : node.virtual_method_impls) {
        if (!seen_impl_names.insert(impl.name).second) {
            throw BuildException("duplicate override method '" + impl.name + "' in AST node type '" + node.name + "'");
        }
        const auto method_it = inherited_virtual_methods.find(impl.name);
        if (method_it == inherited_virtual_methods.end()) {
            throw BuildException("override method '" + impl.name + "' in AST node type '" + node.name +
                                 "' does not exist in its astbase hierarchy");
        }
        if (method_it->second != impl.return_type_name) {
            throw BuildException("override method '" + impl.name + "' in AST node type '" + node.name +
                                 "' uses return type '" + impl.return_type_name + "' but base declares '" +
                                 method_it->second + "'");
        }
...
void ValidateRuleAlternative(const RuleAlternative& alt,
    ...
    if (alt.symbols.empty()) {
        throw BuildException("empty RHS is not currently supported (rule '" + lhs + "')");
    }
    ...
    for (const std::string& symbol : alt.symbols) {
        if (ctx.terminal_set.find(symbol) == ctx.terminal_set.end() &&
            ...
            throw BuildException("unknown symbol '" + symbol + "' in rule '" + lhs + "'");
        }
    }

    const auto check_rhs_ref = [&](std::size_t rhs_index) {
        if (rhs_index == 0 || rhs_index > alt.symbols.size()) {
            throw BuildException("action references out-of-range RHS index $" + std::to_string(rhs_index) +
                                 " in rule '" + lhs + "'");
        }
    };
    ...
    if (alt.action.kind == RuleActionKind::Pass) {
        ...
        if (ctx.terminal_set.find(symbol) != ctx.terminal_set.end()) {
            throw BuildException("Pass($" + std::to_string(alt.action.pass_rhs_index) +
                                 ") cannot forward terminal symbol '" + symbol + "'; use .lexeme in a constructor action");
        }
        return;
    }
    ...
    if (it == ctx.ast_types_by_name.end()) {
        throw BuildException("unknown AST node type in action: " + alt.action.target_node_name);
    }

    const ASTNodeTypeDecl& ast_decl = *it->second;
    if (ast_decl.is_abstract) {
        throw BuildException("action for rule '" + lhs + "' cannot construct astbase '" + ast_decl.name + "'");
    }
    if (ast_decl.fields.size() != alt.action.args.size()) {
        throw BuildException("action for rule '" + lhs + "' constructs '" + ast_decl.name + "' with " +
                             std::to_string(alt.action.args.size()) + " arg(s), but AST node type declares " +
    ...
    for (const ActionArg& arg : alt.action.args) {
        ...
        const bool is_terminal = (ctx.terminal_set.find(symbol) != ctx.terminal_set.end());
        if (arg.kind == ActionArgKind::ChildAST && is_terminal) {
            throw BuildException("action argument $" + std::to_string(arg.rhs_index) +
                                 " refers to terminal symbol '" + symbol + "'; use .lexeme instead");
        }
    ...
    if (ast_decl.field_decls.size() == alt.action.args.size()) {
        for (std::size_t i = 0; i < ast_decl.field_decls.size(); ++i) {
            ...
            if (IsBuiltinAstFieldType(field.type_name)) {
                if (arg.kind != ActionArgKind::ChildLexeme) {
                    throw BuildException("field '" + field.name + "' in AST type '" + ast_decl.name +
                                         "' has builtin text type '" + field.type_name +
                                         "' and must be populated with $N.lexeme");
                }
            } else {
                if (arg.kind != ActionArgKind::ChildAST) {
                    throw BuildException("field '" + field.name + "' in AST type '" + ast_decl.name +
                                         "' has AST type '" + field.type_name +
...
const RuleAlternative& RuleAlternativeForNode(const Stage2SpecAST& spec,
                                              const LR1ParseTable& parse_table,
                                              const CSTNode& node) {
    const FlattenedProduction& production = compiler::parsergen::GetCSTReductionProduction(parse_table, node);
    ...
    if (production.source_alternative_index >= rule.alternatives.size()) {
        throw BuildException("CST reduction has invalid source alternative index");
    }
    return rule.alternatives[production.source_alternative_index];
}

const ASTNodeTypeDecl& FindASTNodeType(const Stage2SpecAST& spec, std::string_view name) {
...
std::unique_ptr<GeneratedASTNode> BuildGeneratedASTNodeRecursive(const CSTNode& node,
                                                                 const LR1ParseTable& parse_table,
    ...
    const RuleAlternative& alt = RuleAlternativeForNode(spec, parse_table, node);
    ...
    if (action.kind == RuleActionKind::Pass) {
        return BuildGeneratedASTNodeRecursive(node.Child(action.pass_rhs_index - 1), parse_table, spec);
    }

    const ASTNodeTypeDecl& ast_decl = FindASTNodeType(spec, action.target_node_name);
    if (ast_decl.is_abstract) {
        throw BuildException("cannot construct abstract AST base '" + ast_decl.name + "'");
    }
    ...
    for (std::size_t i = 0; i < action.args.size(); ++i) {
        ...
        if (arg.kind == ActionArgKind::ChildAST) {
            std::unique_ptr<GeneratedASTNode> child_ast = BuildGeneratedASTNodeRecursive(rhs_node, parse_table, spec);
            if (!child_ast) {
                throw BuildException("action attempted to use terminal RHS symbol as AST child: $" +
                                     std::to_string(arg.rhs_index));
            }
            child_fields.push_back(GeneratedASTNodeChildField{.name = field_name, .value = std::move(child_ast)});
            continue;
        }

        if (!rhs_node.IsTerminal()) {
            throw BuildException(".lexeme action arg referenced nonterminal at runtime");
        }
        text_fields.push_back(GeneratedASTNodeTextField{.name = field_name, .value = std::string(rhs_node.Lexeme())});
    }

    return std::make_unique<GeneratedCustomASTNode>(action.target_node_name, std::move(child_fields), std::move(text_fields));
}
...
std::size_t EmitGeneratedASTGraphvizNode(const GeneratedASTNode& node,
    ...
    for (const GeneratedASTNodeChildField& child_field : node.ChildFields()) {
        const std::size_t child_id = EmitGeneratedASTGraphvizNode(*child_field.value, out, next_id);
...
GeneratedCustomASTNode::GeneratedCustomASTNode(std::string kind_name,
                                               std::vector<GeneratedASTNodeChildField> child_fields,
                                               std::vector<GeneratedASTNodeTextField> text_fields)
    : kind_name_(std::move(kind_name)), child_fields_(std::move(child_fields)), text_fields_(std::move(text_fields)) {}

std::string_view GeneratedCustomASTNode::KindName() const {
    return kind_name_;
}

const std::vector<GeneratedASTNodeChildField>& GeneratedCustomASTNode::ChildFields() const {
    return child_fields_;
}

const std::vector<GeneratedASTNodeTextField>& GeneratedCustomASTNode::TextFields() const {
    return text_fields_;
}

bool GeneratedAST::Empty() const {
    return root == nullptr;
}
...
Stage2SpecAST ParseStage2Spec(std::string_view source_text) {
    try {
        ...
        if (!MatchesReduction(parse_table, root, "File", {"DeclList"})) {
            throw BuildException("unsupported File CST shape in stage2 spec parser");
        }
        ...
        throw BuildException(std::string("failed to parse stage2 spec CST: ") + ex.what());
    }
}
...
void ValidateStage2Spec(const Stage2SpecAST& spec) {
    ...
    if (ctx.nonterminal_set.find(spec.start_symbol) == ctx.nonterminal_set.end()) {
...
LR1CanonicalCollection BuildLR1CanonicalCollection(const Stage2SpecAST& spec) {
    return compiler::parsergen::BuildLR1CanonicalCollection(ToBaseGrammarSpec(spec));
}
...
std::string Stage2SpecASTToGraphvizDot(const Stage2SpecAST& spec, std::string_view graph_name) {
    std::ostringstream out;
    out << "digraph " << compiler::common::SanitizeIdentifier(graph_name, "parser_generator_stage2_spec_ast") << " {\n";
    out << "  rankdir=TB;\n";
    out << "  node [shape=box];\n";
    out << "  root [label=\"Spec\\ngrammar=" << compiler::common::EscapeGraphvizLabel(spec.grammar_name)
        << "\\nstart=" << compiler::common::EscapeGraphvizLabel(spec.start_symbol) << "\"];\n";
    ...
    for (const std::string& token : spec.terminals) {
        out << "  t" << next_id << " [label=\"" << compiler::common::EscapeGraphvizLabel(token) << "\"];\n";
    ...
    for (const ASTNodeTypeDecl& decl : spec.ast_node_types) {
        ...
        if (!decl.fields.empty()) {
            ...
            for (std::size_t i = 0; i < decl.fields.size(); ++i) {
                ...
                const ASTNodeTypeDecl::FieldDecl* field_decl =
                    (i < decl.field_decls.size()) ? &decl.field_decls[i] : nullptr;
                label << decl.fields[i];
        ...
        for (const ASTNodeTypeDecl::VirtualMethodDecl& method : decl.virtual_methods) {
            label << "\\nvirtual " << method.name << "() -> " << method.return_type_name;
        }
        for (const ASTNodeTypeDecl::VirtualMethodImplDecl& impl : decl.virtual_method_impls) {
            label << "\\noverride " << impl.name << "() -> " << impl.return_type_name;
        }
        out << "  a" << next_id << " [label=\"" << compiler::common::EscapeGraphvizLabel(label.str()) << "\"];\n";
    ...
    for (const RuleDefinition& rule : spec.rules) {
        ...
        out << "  r" << rule_id << " [label=\"rule " << compiler::common::EscapeGraphvizLabel(rule.lhs) << "\"];\n";
        out << "  rules -> r" << rule_id << ";\n";
        for (const RuleAlternative& alt : rule.alternatives) {
            std::ostringstream label;
            label << rule.lhs << " -> ";
            ...
            out << "  ralt" << alt_id << " [label=\"" << compiler::common::EscapeGraphvizLabel(label.str()) << "\"];\n";
...
std::string LR1CanonicalCollectionToGraphvizDot(const LR1CanonicalCollection& collection, std::string_view graph_name) {
    return compiler::parsergen1::LR1CanonicalCollectionToGraphvizDot(collection, graph_name);
}

std::string LR1ParseTableToGraphvizDot(const LR1ParseTable& table, std::string_view graph_name) {
...
std::string GeneratedASTToGraphvizDot(const GeneratedAST& ast, std::string_view graph_name) {
    if (ast.Empty()) {
    ...
    std::size_t next_id = 0;
    const std::size_t root_id = EmitGeneratedASTGraphvizNode(ast.Root(), out, next_id);
