...
std::size_t ResolveLegacyFieldIndex(const SymbolTable &symbols,
                                    const ObjectInstancePtr &object,
                                    std::string_view field_name,
                                    std::string_view context) {
    if (!object) {
        throw InterpreterException(std::string(context) + ": null object");
    }
    const SymbolTable::ClassInfo *class_info =
        symbols.FindClass(object->class_name);
    if (class_info == nullptr) {
        throw InterpreterException(std::string(context) + ": unknown class '" +
                                   object->class_name + "'");
    }
    ...
    throw InterpreterException(std::string(context) + ": unknown field '" +
...
double AsNumber(const Value &value, std::string_view context) {
    ...
    if (std::holds_alternative<ObjectInstancePtr>(value)) {
        throw InterpreterException(
            std::string("cannot use object as number in ") +
            std::string(context));
    }
    if (std::holds_alternative<ArrayInstancePtr>(value)) {
        throw InterpreterException(
            std::string("cannot use array as number in ") +
            std::string(context));
    }
    throw InterpreterException(
        std::string("cannot use null value as number in ") +
        std::string(context));
}
...
std::int64_t AsInt64(const Value &value, std::string_view context) {
    ...
    if (!std::isfinite(number)) {
        throw InterpreterException("integer in " + std::string(context) +
                                   " must be finite");
    }
...
std::size_t AsIndex(const Value &value, std::string_view context) {
    ...
    if (!std::isfinite(raw)) {
        throw InterpreterException("index in " + std::string(context) +
                                   " must be finite");
    }
    if (raw < 0.0) {
        throw InterpreterException("index in " + std::string(context) +
...
std::string DescribeExpr(const gen::Expr &expr) {
    ...
    if (const auto *cast_expr = dynamic_cast<const gen::CastExpr *>(&expr)) {
        return "cast(" + DescribeExpr(cast_expr->expr()) + " as " +
               cast_expr->type_name() + ")";
    }
    ...
    if (const auto *method_call =
        ...
        return "call(" + DescribeExpr(method_call->object()) + "." +
               method_call->name() + ")";
    }
    ...
    if (const auto *member = dynamic_cast<const gen::MemberAccess *>(&expr)) {
        return "member(" + DescribeExpr(member->object()) + "." +
               member->member() + ")";
    }
    return "expr";
}
...
std::string DescribeStatement(const gen::Statement &statement) {
    ...
    if (const auto *if_stmt = dynamic_cast<const gen::IfStmt *>(&statement)) {
        std::ostringstream out;
        out << "if (" << DescribeExpr(if_stmt->condition())
            << ") then=" << if_stmt->then_body().size()
    ...
    if (const auto *switch_stmt =
        ...
        return "switch (" + DescribeExpr(switch_stmt->condition()) +
               ") cases=" + std::to_string(switch_stmt->cases().size());
    }
    return "stmt";
}
...
struct RuntimeEnvironment {
    ...
    std::unordered_map<std::string,
                       std::unordered_map<std::string, const gen::MethodDecl *>>
        class_methods;
    std::unordered_map<std::string, const gen::ConstructorDecl *>
        class_constructors;
    ...
    for (const std::unique_ptr<gen::ClassMember> &member :
        ...
        if (const auto *field =
            ...
            if (!seen_fields.insert(field->name()).second) {
                throw InterpreterException("duplicate field '" + field->name() +
        ...
        if (const auto *method =
            ...
            if (method->name() == class_decl.name()) {
                throw InterpreterException("method '" + method->name() +
                                           "' in class '" + class_decl.name() +
                                           "' conflicts with constructor "
                                           "syntax; declare constructors as '" +
                                           class_decl.name() + "(...)'");
            }
            if (!seen_methods.insert(method->name()).second) {
                throw InterpreterException("duplicate method '" +
        ...
        if (const auto *ctor =
            ...
            if (ctor->declared_name() != class_decl.name()) {
                throw InterpreterException(
                    ...
                    "' does not match class name '" + class_decl.name() + "'");
            }
            if (out.constructor_node != nullptr) {
                throw InterpreterException("duplicate constructor in class '" +
                                           class_decl.name() + "'");
            }
        ...
        throw InterpreterException("unsupported class member in class '" +
                                   class_decl.name() + "'");
    }
...
std::vector<std::uint8_t> ToByteBuffer(const Value &value,
    ...
    if (const auto *array = std::get_if<ArrayInstancePtr>(&value)) {
        if (!(*array)) {
            throw InterpreterException(std::string(context) +
...
Value CallBuiltin(std::string_view name, const std::vector<Value> &args) {
    const auto require_count = [&](std::size_t expected) {
        if (args.size() != expected) {
            throw InterpreterException("function '" + std::string(name) +
    ...
    if (name == "read_binary_file") {
        ...
        std::ifstream input(path, std::ios::binary);
        if (!input.is_open()) {
            throw InterpreterException("read_binary_file failed to open: " +
                                       path);
        }
        std::ostringstream buffer;
        buffer << input.rdbuf();
    ...
    if (name == "write_binary_file") {
        ...
        std::ofstream output(path, std::ios::binary | std::ios::trunc);
        if (!output.is_open()) {
            throw InterpreterException("write_binary_file failed to open: " +
                                       path);
        }
        output.write(reinterpret_cast<const char *>(bytes.data()),
                     static_cast<std::streamsize>(bytes.size()));
        if (!output.good()) {
            throw InterpreterException("write_binary_file failed to write: " +
                                       path);
        }
    ...
    if (name == "append_binary_file") {
        ...
        std::ofstream output(path, std::ios::binary | std::ios::app);
        if (!output.is_open()) {
            throw InterpreterException("append_binary_file failed to open: " +
                                       path);
        }
        output.write(reinterpret_cast<const char *>(bytes.data()),
                     static_cast<std::streamsize>(bytes.size()));
        if (!output.good()) {
            throw InterpreterException("append_binary_file failed to write: " +
                                       path);
        }
        return static_cast<double>(bytes.size());
    }
    ...
    if (name == "file_size") {
        ...
        if (error) {
            throw InterpreterException("file_size failed for '" + path +
                                       "': " + error.message());
        }
    ...
    if (name == "push") {
        ...
        if (array == nullptr || !(*array)) {
            throw InterpreterException(
                "push expects an array as first argument");
        }
...
bool AssignExistingVariable(std::string_view name, Value value,
...
StatementResult ExecuteStatement(const gen::Statement &statement,
                                 RuntimeEnvironment &env);
StatementResult ExecuteStatementList(
    const std::vector<std::unique_ptr<gen::Statement>> &statements,
    RuntimeEnvironment &env, bool create_scope = false);

Value AssignToTarget(const gen::Expr &target, Value value,
                     RuntimeEnvironment &env) {
    if (const auto *identifier =
            dynamic_cast<const gen::Identifier *>(&target)) {
        if (!AssignExistingVariable(identifier->name(), value, env)) {
            throw InterpreterException("assignment to unknown identifier: " +
                                       identifier->name());
        }
        return value;
    }
    ...
    if (const auto *member = dynamic_cast<const gen::MemberAccess *>(&target)) {
        ...
        if (object == nullptr || !(*object)) {
            throw InterpreterException(
                "member assignment requires an object value");
        }
        const std::size_t field_index = ResolveLegacyFieldIndex(
            env.symbols, *object, member->member(), "member assignment");
        if (field_index >= (*object)->memory.size()) {
            throw InterpreterException(
                "member assignment out of range for type '" +
                (*object)->class_name + "'");
        }
    ...
    if (const auto *index = dynamic_cast<const gen::IndexAccess *>(&target)) {
        ...
        const std::size_t at =
            AsIndex(EvaluateExpr(index->index(), env), "index assignment");
        if (at >= (*array)->elements.size()) {
            throw InterpreterException("index assignment out of range: " +
                                       std::to_string(at));
        }
...
const gen::MethodDecl *FindMethodOnClass(const RuntimeEnvironment &env,
...
const gen::ConstructorDecl *
FindConstructorOnClass(const RuntimeEnvironment &env,
                       std::string_view class_name) {
...
Value CallUserFunction(const gen::FunctionDecl &function,
    ...
    struct FrameGuard {
        ...
        ~FrameGuard() { env_ref.scopes.pop_back(); }
    } guard{env};

    const StatementResult result =
        ExecuteStatementList(function.body(), env, false);
    if (result.broke) {
...
Value CallMethod(const ObjectInstancePtr &self_object,
                 const gen::MethodDecl &method, const std::vector<Value> &args,
                 RuntimeEnvironment &env) {
    if (!self_object) {
        throw InterpreterException("cannot call method '" + method.name() +
                                   "' on null object");
    }
    if (method.params().size() != args.size()) {
        throw InterpreterException(
            "method '" + self_object->class_name + "." + method.name() +
            "' expects " + std::to_string(method.params().size()) +
    ...
    const StatementResult result =
        ExecuteStatementList(method.body(), env, false);
    if (result.broke) {
        throw InterpreterException("break used outside loop or switch");
    }
    return result.value;
...
Value CallConstructor(const ObjectInstancePtr &self_object,
                      const gen::ConstructorDecl &ctor,
                      const std::vector<Value> &args, RuntimeEnvironment &env) {
    if (!self_object) {
        throw InterpreterException("cannot call constructor '" +
                                   ctor.declared_name() + "' on null object");
    }
    if (ctor.params().size() != args.size()) {
        throw InterpreterException(
            "constructor '" + self_object->class_name + "(" +
    ...
    const StatementResult result =
        ExecuteStatementList(ctor.body(), env, false);
    if (result.broke) {
        throw InterpreterException("break used outside loop or switch");
    }
    return result.value;
...
Value CallMethodByName(const ObjectInstancePtr &self_object,
                       ...
                       RuntimeEnvironment &env) {
    if (!self_object) {
        throw InterpreterException("cannot call method '" +
                                   std::string(method_name) +
                                   "' on null object");
    }
    const gen::MethodDecl *method =
        FindMethodOnClass(env, self_object->class_name, method_name);
    if (method == nullptr) {
        throw InterpreterException("unknown method '" +
                                   std::string(method_name) + "' on type '" +
                                   self_object->class_name + "'");
    }
    return CallMethod(self_object, *method, args, env);
}

bool TryCallOperator(const Value &target, std::string_view method_name,
                     const std::vector<Value> &args, RuntimeEnvironment &env,
                     Value &out) {
...
bool TryCallOperatorBool(const Value &target, std::string_view method_name,
                         const std::vector<Value> &args,
...
Value ApplyCompoundOperation(std::string_view op, const Value &lhs,
                             const Value &rhs, RuntimeEnvironment &env) {
    ...
    throw InterpreterException("unsupported compound assignment operator: " +
                               std::string(op));
}

Value EvaluateEmbeddedExpression(std::string_view expression_text,
                                 RuntimeEnvironment &env) {
    ...
    if (items.size() != 1 || items.front() == nullptr) {
        throw InterpreterException("formatted-string expression must parse as "
    ...
    if (expr_statement == nullptr) {
        throw InterpreterException(
            "formatted-string expression must be an expression: " + trimmed);
    }

    return EvaluateExpr(expr_statement->expr(), env);
}

std::string EvaluateFormattedStringLiteral(std::string_view literal,
                                           RuntimeEnvironment &env) {
    ...
    while (i < body.size()) {
        ...
        if (c == '{') {
            ...
            const std::string_view expression_text =
                TrimWhitespace(body.substr(i + 1, close - (i + 1)));
            if (expression_text.empty()) {
                throw InterpreterException(
                    "empty {} expression in formatted string");
            }

            out += ValueToStringInternal(
                EvaluateEmbeddedExpression(expression_text, env));
            i = close + 1;
            continue;
        }
...
std::vector<Value>
EvaluateArgs(const std::vector<std::unique_ptr<gen::Expr>> &args,
             RuntimeEnvironment &env) {
...
Value EvaluateExpr(const gen::Expr &expr, RuntimeEnvironment &env) {
    ...
    if (const auto *compound_assign_expr =
        ...
        return AssignToTarget(compound_assign_expr->target(),
                              ApplyCompoundOperation(compound_assign_expr->op(),
                                                     current, rhs, env),
                              env);
    }
    if (const auto *array_literal =
    ...
    if (const auto *index = dynamic_cast<const gen::IndexAccess *>(&expr)) {
        const Value object_value = EvaluateExpr(index->object(), env);
        const std::size_t at =
            AsIndex(EvaluateExpr(index->index(), env), "index access");
        ...
        if (const auto *array = std::get_if<ArrayInstancePtr>(&object_value)) {
            ...
            if (at >= (*array)->elements.size()) {
                throw InterpreterException("index access out of range: " +
                                           std::to_string(at));
            }
        ...
        if (const auto *text = std::get_if<std::string>(&object_value)) {
            if (at >= text->size()) {
                throw InterpreterException("string index out of range: " +
                                           std::to_string(at));
            }
    ...
    if (const auto *div = dynamic_cast<const gen::Divide *>(&expr)) {
        ...
        Value overloaded_result;
        if (TryCallOperator(lhs, "__div__", {rhs_value}, env,
                            overloaded_result)) {
            return overloaded_result;
        }
        if (TryCallOperator(rhs_value, "__rdiv__", {lhs}, env,
                            overloaded_result)) {
            return overloaded_result;
        }
    ...
    if (const auto *idiv = dynamic_cast<const gen::IntDivide *>(&expr)) {
        ...
        Value overloaded_result;
        if (TryCallOperator(lhs, "__floordiv__", {rhs_value}, env,
                            overloaded_result)) {
            return overloaded_result;
        }
        if (TryCallOperator(rhs_value, "__rfloordiv__", {lhs}, env,
                            overloaded_result)) {
            return overloaded_result;
        }
        const double rhs = AsNumber(rhs_value, "//");
    ...
    if (const auto *member = dynamic_cast<const gen::MemberAccess *>(&expr)) {
        ...
        const std::size_t field_index = ResolveLegacyFieldIndex(
            env.symbols, *object, member->member(), "member access");
        if (field_index >= (*object)->memory.size()) {
            throw InterpreterException("member access out of range for type '" +
                                       (*object)->class_name + "'");
        }
    ...
    if (const auto *call = dynamic_cast<const gen::Call *>(&expr)) {
        ...
        if (const SymbolTable::ClassInfo *class_info =
                env.symbols.FindClass(call->name())) {
            throw InterpreterException(
                "type '" + class_info->name + "' is not callable; use 'new " +
                class_info->name + "(...)' for construction");
        }
    ...
    if (const auto *new_expr = dynamic_cast<const gen::NewExpr *>(&expr)) {
        ...
        const SymbolTable::ClassInfo *class_info =
            env.symbols.FindClass(new_expr->name());
        if (class_info == nullptr) {
        ...
        const gen::ConstructorDecl *ctor =
            FindConstructorOnClass(env, class_info->name);
        if (ctor != nullptr) {
...
StatementResult ExecuteStatement(const gen::Statement &statement,
    ...
    if (const auto *return_stmt =
            dynamic_cast<const gen::ReturnStmt *>(&statement)) {
        return StatementResult{.returned = true,
                               .value = EvaluateExpr(return_stmt->expr(), env)};
    }
    if (dynamic_cast<const gen::BreakStmt *>(&statement) != nullptr) {
    ...
    if (const auto *expr_stmt =
            dynamic_cast<const gen::ExprStmt *>(&statement)) {
        return StatementResult{.returned = false,
                               .value = EvaluateExpr(expr_stmt->expr(), env)};
    }
    if (const auto *if_stmt = dynamic_cast<const gen::IfStmt *>(&statement)) {
    ...
    if (const auto *while_stmt =
        ...
        while (IsTruthy(EvaluateExpr(while_stmt->condition(), env))) {
            const StatementResult body =
                ExecuteStatementList(while_stmt->body(), env, true);
            if (body.returned) {
                return body;
            }
            if (body.broke) {
    ...
    if (const auto *for_stmt = dynamic_cast<const gen::ForStmt *>(&statement)) {
        ...
        struct ScopeGuard {
            RuntimeEnvironment &env_ref;
            ~ScopeGuard() { env_ref.scopes.pop_back(); }
        } guard{env};
        ...
        while (IsTruthy(EvaluateExpr(for_stmt->condition(), env))) {
            const StatementResult body =
                ExecuteStatementList(for_stmt->body(), env, true);
            if (body.returned) {
                return body;
            }
            if (body.broke) {
    ...
    if (const auto *switch_stmt =
            dynamic_cast<const gen::SwitchStmt *>(&statement)) {
        const auto execute_switch_body =
            [&](const std::vector<std::unique_ptr<gen::Statement>> &body)
            -> StatementResult {
            const StatementResult branch =
                ExecuteStatementList(body, env, true);
            if (branch.returned) {
                return branch;
            }
            if (branch.broke) {
                return StatementResult{
                    .returned = false, .broke = false, .value = branch.value};
            }
            return branch;
        };
        ...
        for (const std::unique_ptr<gen::SwitchCase> &switch_case :
            ...
            if (ValueEquals(condition,
                            EvaluateExpr(switch_case->match(), env))) {
...
void CollectTopLevelGlobalFromStatement(const gen::Statement &statement,
                                        SymbolTable &symbols) {
    if (const auto *let_stmt = dynamic_cast<const gen::LetStmt *>(&statement)) {
...
ProgramAnnotation AnnotateProgram(const AST &ast) {
    ...
    for (const std::unique_ptr<gen::Item> &item : program.items().items()) {
        ...
        if (const auto *function =
            ...
            out.flattened_items.push_back(
                "fn " + function->name() + "(" +
                JoinCommaSeparated(function->params()) + ")");
            if (!out.symbols.DeclareFunction(function->name())) {
                throw InterpreterException("duplicate function declaration: " +
                                           function->name());
            }
        ...
        if (const auto *module_decl =
                dynamic_cast<const gen::ModuleDecl *>(item.get())) {
            out.flattened_items.push_back("mod " + module_decl->name());
            continue;
        }

        if (const auto *use_decl =
                dynamic_cast<const gen::UseDecl *>(item.get())) {
            out.flattened_items.push_back("use " + use_decl->name());
            continue;
        }
        ...
        if (const auto *class_decl =
            ...
            const ClassMemberSummary summary =
                SummarizeClassMembers(*class_decl);
            if (!out.symbols.DeclareClass(class_decl->name(), summary.fields,
                                          summary.methods)) {
                throw InterpreterException("duplicate class declaration: " +
                                           class_decl->name());
            }
        ...
        if (const auto *top_stmt =
            ...
            CollectTopLevelGlobalFromStatement(top_stmt->statement(),
                                               out.symbols);
            out.flattened_items.push_back(
                DescribeStatement(top_stmt->statement()));
            continue;
        }
    }
...
void RegisterProgramDeclarations(const gen::Program &program,
                                 RuntimeEnvironment &env,
    ...
    for (const std::unique_ptr<gen::Item> &item : program.items().items()) {
        ...
        if (const auto *class_decl =
            ...
            const ClassMemberSummary summary =
                SummarizeClassMembers(*class_decl);
            if (!env.symbols.DeclareClass(class_decl->name(), summary.fields,
                                          summary.methods)) {
                throw InterpreterException(
                    "duplicate class declaration: " + class_decl->name() +
                    " (" + std::string(unit_name) + ")");
            }

            auto &method_map = env.class_methods[class_decl->name()];
            for (const auto &[method_name, method_node] :
                 summary.method_nodes) {
                if (!method_map.emplace(method_name, method_node).second) {
                    throw InterpreterException(
                        "duplicate method declaration: " + class_decl->name() +
            ...
            if (summary.constructor_node != nullptr) {
                if (!env.class_constructors
                         .emplace(class_decl->name(), summary.constructor_node)
                         .second) {
                    throw InterpreterException(
                        "duplicate constructor declaration for class '" +
                        class_decl->name() + "' (" + std::string(unit_name) +
                        ")");
                }
            }
        }
...
Value ExecuteTopLevelStatements(const gen::Program &program,
                                RuntimeEnvironment &env,
                                std::string_view unit_name,
    ...
    for (const std::unique_ptr<gen::Item> &item : program.items().items()) {
        if (const auto *top_stmt =
                dynamic_cast<const gen::TopStatement *>(item.get())) {
            const StatementResult result =
                ExecuteStatement(top_stmt->statement(), env);
            ...
            if (result.returned) {
                ...
                throw InterpreterException(
                    "top-level return is not allowed in " +
                    std::string(unit_name));
            }
            if (result.broke) {
                throw InterpreterException(
                    "top-level break is not allowed in " +
                    std::string(unit_name));
            }
        }
    }
...
void LoadStandardLibrary(RuntimeEnvironment &env) {
    ...
    for (const LoadedProgramUnit &unit : env.loaded_units) {
        (void)ExecuteTopLevelStatements(RequireProgram(unit.ast), env,
                                        unit.name, false);
    }
}
