...
std::string BuildIRAssemblyDump(
    const std::vector<compiler::ir::ProgramUnit> &prelude_units,
    const compiler::ir::Program &program, std::string_view unit_name) {
...
int RunFileMode(const std::filesystem::path &input_path,
    ...
    const compiler::interpreter::Value result =
        compiler::interpreter::ExecuteIRProgram(std::move(optimized_ir_program),
                                                std::move(optimized_stdlib_ir_units),
                                                false);
