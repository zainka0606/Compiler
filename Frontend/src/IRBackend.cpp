...
const gen::Program &RequireProgram(const AST &ast) {
    if (ast.Empty()) {
        throw FrontendPipelineException("program AST is empty");
    }
    const auto *program = dynamic_cast<const gen::Program *>(&ast.Root());
    if (program == nullptr) {
        throw FrontendPipelineException("program AST root is not Program");
    }
    return *program;
}
...
std::string_view TrimWhitespace(const std::string_view text) {
    std::size_t begin = 0;
...
std::string DecodeEscapedText(const std::string_view text,
    ...
    for (std::size_t i = 0; i < text.size(); ++i) {
        ...
        if (i + 1 >= text.size()) {
            throw FrontendPipelineException(std::string("dangling escape in ") +
                                            std::string(context));
        }
...
std::string ParseQuotedLiteral(const std::string_view text, const char quote,
                               const std::string_view context) {
    if (text.size() < 2 || text.front() != quote || text.back() != quote) {
        throw FrontendPipelineException(std::string("invalid ") +
                                        std::string(context) +
...
class IRFunctionCodegen {
  public:
    explicit IRFunctionCodegen(ir::Function &function,
                               const bool module_scope_globals = false)
  ...
  private:
    ...
    ResolveLocalSlot(const std::string_view name) const {
    ...
    void EmitJumpIfNotTerminated(const ir::BlockId target) {
        if (!builder_.HasTerminator(builder_.Current())) {
            builder_.SetTerminator(ir::JumpTerm{.target = target});
        }
    }

    ir::ValueRef EmitUnary(const ir::UnaryOp op, ir::ValueRef value) {
        const ir::LocalId dst = builder_.NewTemp();
        builder_.Emit(ir::UnaryInst{.dst = dst, .op = op, .value = value});
        return ir::ValueRef::Temp(dst);
    }

    ir::ValueRef EmitBinary(const ir::BinaryOp op, ir::ValueRef lhs,
                            ir::ValueRef rhs) {
        const ir::LocalId dst = builder_.NewTemp();
        builder_.Emit(
    ...
    ir::ValueRef EmitNewExpr(const gen::NewExpr &new_expr,
                             const bool force_heap) {
        const ir::LocalId dst = builder_.NewTemp();
        std::vector<ir::ValueRef> args;
        args.reserve(new_expr.args().size());
        for (const std::unique_ptr<gen::Expr> &arg : new_expr.args()) {
            if (!arg) {
                throw FrontendPipelineException("null constructor argument node");
            }
            args.push_back(EmitExpr(*arg));
        }
        bool use_heap = force_heap;
        if (!use_heap) {
            if (new_expr.storage() == "heap") {
                use_heap = true;
            } else if (!new_expr.storage().empty() &&
                       new_expr.storage() != "stack") {
                throw FrontendPipelineException("unsupported new storage mode: " +
                                                new_expr.storage());
            }
        }
        builder_.Emit(ir::CallInst{
            .dst = dst,
            .callee = new_expr.name() + (use_heap ? ".__new_heap" : ".__new"),
            .args = std::move(args),
        });
        return ir::ValueRef::Temp(dst);
    }
    ...
    LValue EmitLValue(const gen::Expr &target) {
        ...
        throw FrontendPipelineException("invalid assignment target");
    }
    ...
    ir::BinaryOp BinaryOpForCompound(const std::string_view op) {
        if (op == "+=") {
    ...
    ir::ValueRef EmitEmbeddedExpression(const std::string_view expression_text) {
        ...
        if (items.size() != 1 || items.front() == nullptr) {
            throw FrontendPipelineException(
                "formatted-string expression must parse as one expression "
                "statement: " +
                trimmed);
        }
    ...
    ir::ValueRef EmitFormattedString(const std::string_view literal) {
        if (literal.size() < 3 || literal[0] != 'f' || literal[1] != '"' ||
            literal.back() != '"') {
            throw FrontendPipelineException(
                "invalid formatted string literal: " + std::string(literal));
        }
        ...
        std::function append_part =
            [&](const ir::ValueRef &part) {
                const ir::LocalId next = builder_.NewTemp();
                std::vector<ir::ValueRef> args;
                args.reserve(2);
                args.push_back(ir::ValueRef::Temp(out_temp));
                args.push_back(part);
                builder_.Emit(ir::CallInst{
                    .dst = next,
                    .callee = "stringConcat",
    ...
    ir::ValueRef EmitExpr(const gen::Expr &expr) {
        ...
        if (const auto *bool_literal =
            ...
            throw FrontendPipelineException("invalid bool literal: " +
                                            bool_literal->value());
        }
        if (const auto *identifier =
        ...
        if (const auto *method_call =
            ...
            for (const std::unique_ptr<gen::Expr> &arg : method_call->args()) {
                if (!arg) {
                    throw FrontendPipelineException("null call argument node");
                }
                args.push_back(EmitExpr(*arg));
            }
        ...
        if (const auto *new_expr = dynamic_cast<const gen::NewExpr *>(&expr)) {
            return EmitNewExpr(*new_expr, false);
        }
    ...
    void EmitStatement(const gen::Statement &statement) {
        if (const auto *let_stmt =
            ...
            ir::ValueRef value;
            if (IsModuleScope()) {
                if (const auto *new_expr =
                        dynamic_cast<const gen::NewExpr *>(&let_stmt->expr())) {
                    value = EmitNewExpr(*new_expr, true);
                } else {
                    value = EmitExpr(let_stmt->expr());
                }
            } else {
                value = EmitExpr(let_stmt->expr());
            }
        ...
        throw FrontendPipelineException(
            "unsupported statement node in IR lowering");
    }
};
    ...
    for (std::size_t i = 0; i < function.params.size(); ++i) {
        ...
        if (is_method && (param == "self" || param == "this")) {
            throw FrontendPipelineException(
                ...
                "' parameter names cannot use reserved identifier '" + param +
                "'");
        }
...
struct ConstructorCall {
    ...
    bool forward_allocator_args = false;
...
struct LoweredClass {
    ...
    std::vector<std::string> allocator_params;
    std::vector<std::string> allocator_param_types;
    std::vector<ConstructorCall> constructor_chain;
    std::unordered_map<std::string, std::vector<std::string>>
        method_param_types;
};

ir::Function
CompileClassAllocatorFunction(std::string class_name,
                              std::string allocator_function_name,
                              std::string allocator_builtin_name,
                              std::vector<std::string> ctor_params,
                              std::vector<std::string> ctor_param_types,
                              std::vector<ConstructorCall> constructor_chain) {
    if (ctor_params.size() != ctor_param_types.size()) {
        throw FrontendPipelineException("allocator for class '" + class_name +
                                        "' has mismatched parameter type metadata");
    }

    ir::Function function;
    function.name = std::move(allocator_function_name);
    function.params = std::move(ctor_params);
    function.param_types = std::move(ctor_param_types);
    function.param_slots.reserve(function.params.size());
    for (std::size_t i = 0; i < function.params.size(); ++i) {
        const ir::SlotId slot = function.next_slot++;
        function.param_slots.push_back(slot);
        function.slot_names.push_back(function.params[i]);
        function.slot_types.push_back(function.param_types[i]);
    }
    function.blocks.push_back(ir::BasicBlock{.id = 0, .label = "entry"});
    function.entry = 0;
    ...
    const ir::LocalId result_temp = builder.NewTemp();
    builder.Emit(ir::CallInst{
        .dst = result_temp,
        .callee = std::move(allocator_builtin_name),
        .args = {ir::ValueRef::String(class_name)},
    });

    for (const ConstructorCall &ctor : constructor_chain) {
        std::vector<ir::ValueRef> ctor_args;
        ctor_args.reserve(1 + (ctor.forward_allocator_args
                                   ? function.param_slots.size()
                                   : 0U));
        ctor_args.push_back(ir::ValueRef::Temp(result_temp));

        if (ctor.forward_allocator_args) {
            if (ctor.parameter_count != function.param_slots.size()) {
                throw FrontendPipelineException(
                    "constructor forwarding mismatch in allocator '" +
                    function.name + "' for class '" + class_name + "'");
            }
            for (const ir::SlotId slot : function.param_slots) {
                const ir::LocalId param_temp = builder.NewTemp();
                builder.Emit(
                    ir::LoadLocalInst{.dst = param_temp, .slot = slot});
                ctor_args.push_back(ir::ValueRef::Temp(param_temp));
            }
        } else if (ctor.parameter_count != 0) {
            throw FrontendPipelineException(
                "constructor '" + ctor.function_name +
                "' requires arguments but no forwarding is available in allocator '" +
                function.name + "'");
        }

        const ir::LocalId ctor_result = builder.NewTemp();
        builder.Emit(ir::CallInst{
            .dst = ctor_result,
            .callee = ctor.function_name,
            .args = std::move(ctor_args),
        });
    }

    builder.SetTerminator(
        ir::ReturnTerm{.value = ir::ValueRef::Temp(result_temp)});
    return function;
}
...
AST ParseProgramFile(const std::filesystem::path &source_path) {
    try {
        return ParseProgram(common::ReadTextFile(source_path));
    } catch (const std::exception &ex) {
        throw FrontendPipelineException(
...
ir::Program CompileProgramToIR(const AST &ast) {
    try {
        ...
        std::unordered_map<std::string, const gen::ClassDecl *> class_decls;
        std::vector<std::string> class_order;
        class_order.reserve(program.items().items().size());
        for (const std::unique_ptr<gen::Item> &item : program.items().items()) {
            if (!item) {
                continue;
            }
            const auto *class_decl = dynamic_cast<const gen::ClassDecl *>(item.get());
            if (class_decl == nullptr) {
                continue;
            }
            if (!class_decls.emplace(class_decl->name(), class_decl).second) {
                throw FrontendPipelineException(
                    "duplicate class '" + class_decl->name() + "'");
            }
            class_order.push_back(class_decl->name());
        }
        ...
        std::unordered_map<std::string, ClassBuildState> class_states;
        class_states.reserve(class_decls.size());

        std::function<const LoweredClass &(const std::string &)> build_class;
        build_class = [&](const std::string &class_name) -> const LoweredClass & {
            if (const auto state_it = class_states.find(class_name);
                state_it != class_states.end() && state_it->second.built) {
                return state_it->second.value;
            }

            const auto class_decl_it = class_decls.find(class_name);
            if (class_decl_it == class_decls.end()) {
                throw FrontendPipelineException("unknown class '" + class_name +
                                                "' while resolving inheritance");
            }

            ClassBuildState &state = class_states[class_name];
            if (state.building) {
                throw FrontendPipelineException(
                    "cyclic class inheritance detected at '" + class_name + "'");
            }
            state.building = true;

            const gen::ClassDecl &class_decl = *class_decl_it->second;
            LoweredClass lowered;
            lowered.class_info.name = class_name;

            if (!class_decl.base_name().empty()) {
                const auto base_decl_it = class_decls.find(class_decl.base_name());
                if (base_decl_it == class_decls.end()) {
                    throw FrontendPipelineException(
                        "base class '" + class_decl.base_name() +
                        "' for class '" + class_name +
                        "' is not declared in this module");
                }
                const LoweredClass &base = build_class(class_decl.base_name());
                lowered.class_info.base_class = base.class_info.name;
                lowered.class_info.fields = base.class_info.fields;
                lowered.class_info.field_types = base.class_info.field_types;
                lowered.class_info.field_offsets = base.class_info.field_offsets;
                lowered.class_info.method_slots = base.class_info.method_slots;
                lowered.class_info.vtable_functions =
                    base.class_info.vtable_functions;
                lowered.class_info.method_functions =
                    base.class_info.method_functions;
                lowered.allocator_params = base.allocator_params;
                lowered.allocator_param_types = base.allocator_param_types;
                lowered.constructor_chain = base.constructor_chain;
                lowered.method_param_types = base.method_param_types;
            }
            ...
            std::vector<std::string> constructor_param_types;
            ...
            for (const std::unique_ptr<gen::ClassMember> &member :
                ...
                if (const auto *field =
                        dynamic_cast<const gen::FieldDecl *>(member.get())) {
                    if (!seen_fields.insert(field->name()).second) {
                        throw FrontendPipelineException(
                            "duplicate field '" + field->name() +
                            "' in class '" + class_name + "'");
                    }
                    if (lowered.class_info.field_offsets.contains(field->name())) {
                        throw FrontendPipelineException(
                            "field '" + field->name() + "' in class '" +
                            class_name +
                            "' conflicts with an inherited field");
                    }
                    lowered.class_info.fields.push_back(field->name());
                    lowered.class_info.field_types.push_back(field->type_name());
                    lowered.class_info.field_offsets[field->name()] =
                        static_cast<ir::SlotId>(lowered.class_info.fields.size());
                    continue;
                }

                if (const auto *method =
                        dynamic_cast<const gen::MethodDecl *>(member.get())) {
                    ...
                    const auto inherited_signature =
                        lowered.method_param_types.find(method->name());
                    if (inherited_signature != lowered.method_param_types.end() &&
                        inherited_signature->second != method->param_types()) {
                        throw FrontendPipelineException(
                            "method '" + class_name + "." + method->name() +
                            "' overrides with incompatible parameter list");
                    }

                    const std::string function_name =
                        class_name + "." + method->name();
                    const auto existing_slot_it =
                        lowered.class_info.method_slots.find(method->name());
                    if (existing_slot_it != lowered.class_info.method_slots.end()) {
                        const ir::SlotId method_slot = existing_slot_it->second;
                        if (method_slot >=
                            lowered.class_info.vtable_functions.size()) {
                            throw FrontendPipelineException(
                                "invalid inherited vtable slot while lowering '" +
                                class_name + "." + method->name() + "'");
                        }
                        lowered.class_info.vtable_functions[method_slot] =
                            function_name;
                    } else {
                        const ir::SlotId method_slot = static_cast<ir::SlotId>(
                            lowered.class_info.vtable_functions.size());
                        lowered.class_info.method_slots[method->name()] =
                            method_slot;
                        lowered.class_info.vtable_functions.push_back(
                            function_name);
                    }
                    lowered.class_info.method_functions[method->name()] =
                        function_name;
                    lowered.method_param_types[method->name()] =
                        method->param_types();

                    lowered.functions.push_back(CompileFunctionBody(
                        function_name, method->params(), method->param_types(),
                        method->body(), true, class_name, method->name()));
                    continue;
                }

                if (const auto *ctor =
                        dynamic_cast<const gen::ConstructorDecl *>(member.get())) {
                    if (ctor->declared_name() != class_name) {
                        throw FrontendPipelineException(
                            "constructor '" + ctor->declared_name() +
                            "' does not match class name '" + class_name + "'");
                    }
                    if (seen_constructor) {
                        throw FrontendPipelineException(
                            "duplicate constructor in class '" + class_name + "'");
                    }
                    seen_constructor = true;

                    constructor_function_name = class_name + ".__ctor";
                    lowered.class_info.constructor_function =
                        constructor_function_name;
                    constructor_params = ctor->params();
                    constructor_param_types = ctor->param_types();
                    lowered.functions.push_back(CompileFunctionBody(
                        constructor_function_name, ctor->params(),
                        ctor->param_types(), ctor->body(), true, class_name,
                        "__ctor"));
                    continue;
                }

                throw FrontendPipelineException(
                    "unsupported class member in class '" + class_name + "'");
            }

            const bool inherited_forwarded_ctor =
                std::any_of(lowered.constructor_chain.begin(),
                            lowered.constructor_chain.end(),
                            [](const ConstructorCall &call) {
                                return call.forward_allocator_args;
                            });
            if (seen_constructor) {
                if (inherited_forwarded_ctor) {
                    throw FrontendPipelineException(
                        "class '" + class_name +
                        "' defines a constructor but base class constructor "
                        "requires arguments; add explicit base-constructor "
                        "support before overriding");
                }
                lowered.allocator_params = constructor_params;
                lowered.allocator_param_types = constructor_param_types;
                lowered.constructor_chain.push_back(ConstructorCall{
                    .function_name = constructor_function_name,
                    .forward_allocator_args = !constructor_params.empty(),
                    .parameter_count = constructor_params.size(),
                });
            }

            if (lowered.allocator_params.size() !=
                lowered.allocator_param_types.size()) {
                throw FrontendPipelineException(
                    "allocator signature mismatch in class '" + class_name +
                    "'");
            }

            state.value = std::move(lowered);
            state.built = true;
        ...
        for (const std::unique_ptr<gen::Item> &item : program.items().items()) {
            ...
            if (dynamic_cast<const gen::ModuleDecl *>(item.get()) != nullptr) {
                continue;
            }

            if (dynamic_cast<const gen::UseDecl *>(item.get()) != nullptr) {
                continue;
            }

            if (dynamic_cast<const gen::ClassDecl *>(item.get()) != nullptr) {
                continue;
            }
        ...
        for (const std::string &class_name : class_order) {
            (void)build_class(class_name);
        }

        for (const std::string &class_name : class_order) {
            auto state_it = class_states.find(class_name);
            if (state_it == class_states.end() || !state_it->second.built) {
                throw FrontendPipelineException(
                    "internal error: class '" + class_name +
                    "' was not lowered");
            }
            LoweredClass &lowered = state_it->second.value;

            for (ir::Function &function : lowered.functions) {
                out.functions.push_back(std::move(function));
            }

            out.functions.push_back(CompileClassAllocatorFunction(
                class_name, class_name + ".__new", "stack_alloc",
                lowered.allocator_params, lowered.allocator_param_types,
                lowered.constructor_chain));
            out.functions.push_back(CompileClassAllocatorFunction(
                class_name, class_name + ".__new_heap", "alloc",
                lowered.allocator_params, lowered.allocator_param_types,
                lowered.constructor_chain));
            out.classes.push_back(std::move(lowered.class_info));
        }
