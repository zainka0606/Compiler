...
namespace {
using example::lr0input::ExampleInputLexer;
using example::lr0input::ExampleInputTokenKind;
using InputToken = example::lr0input::Token;
...
const compiler::lr0::LR0Action *
FindAction(const compiler::lr0::LR0ParseTable &table, std::size_t state_index,
...
const std::size_t *FindGoto(const compiler::lr0::LR0ParseTable &table,
...
ParseTree ParseInputWithLR0(const compiler::lr0::LR0ParseTable &table,
                            const std::vector<InputToken> &tokens) {
    if (!table.conflicts.empty()) {
        throw std::runtime_error("cannot parse with conflicting LR(0) table (" +
                                 std::to_string(table.conflicts.size()) +
                                 " conflict(s))");
    }
    if (table.canonical_collection.states.empty()) {
        throw std::runtime_error("LR(0) table has no states");
    }
    ...
    while (true) {
        ...
        const compiler::lr0::LR0Action *action =
        ...
        switch (action->kind) {
        case compiler::lr0::LR0ActionKind::Shift: {
            if (action->target_state == compiler::lr0::kInvalidIndex) {
        ...
        case compiler::lr0::LR0ActionKind::Reduce: {
        ...
        case compiler::lr0::LR0ActionKind::Accept: {
...
int main(int argc, char **argv) {
    if (argc != 3 && argc != 4) {
        std::cerr << "usage: ExampleLR0ParserDriver <grammar-spec> <input> "
                     "[ast-dot-output]\n";
        return 2;
    }
    ...
    try {
        ...
        const compiler::lr0::GrammarSpecAST grammar =
            compiler::lr0::ParseGrammarSpec(grammar_text);
        const compiler::lr0::LR0ParseTable table =
            compiler::lr0::BuildLR0ParseTable(grammar);

        ExampleInputLexer lexer(input_text);
        const std::vector<InputToken> tokens = lexer.Tokenize();

        const ParseTree parse_tree = ParseInputWithLR0(table, tokens);
        WriteTextFile(output_path, ParseTreeToGraphvizDot(parse_tree));
        std::cout << "Wrote AST DOT to " << output_path.string() << "\n";
        return 0;
    } catch (const compiler::lr0::ParseException &ex) {
