...
using GeneratedParser =
    generated::MiniLangInterpreter::MiniLangInterpreterParser;
...
double ParseNumberFromText(std::string_view text, std::string_view context) {
    ...
    if (end == nullptr || *end != '\0') {
        throw InterpreterException(std::string("invalid number in ") +
...
std::string DescribeStatement(const gen::Statement &statement) {
    ...
    if (const auto *for_stmt = dynamic_cast<const gen::ForStmt *>(&statement)) {
        return "for (" + DescribeForInit(for_stmt->init()) + "; " +
...
Value LookupVariable(std::string_view name, const RuntimeEnvironment &env) {
    for (auto it = env.local_frames.rbegin(); it != env.local_frames.rend();
         ++it) {
    ...
    if (env.symbols.globals.find(std::string(name)) !=
        env.symbols.globals.end()) {
        throw InterpreterException(
            "global variable declared but not initialized at use: " +
            std::string(name));
    }
...
void ExecuteForInit(const gen::ForInit &init, RuntimeEnvironment &env) {
    if (const auto *let_init = dynamic_cast<const gen::ForInitLet *>(&init)) {
        AssignVariable(let_init->name(), EvaluateExpr(let_init->expr(), env),
                       env);
        return;
    }
    if (const auto *expr_init = dynamic_cast<const gen::ForInitExpr *>(&init)) {
...
Value EvaluateExpr(const gen::Expr &expr, RuntimeEnvironment &env) {
    ...
    if (const auto *sub = dynamic_cast<const gen::Subtract *>(&expr)) {
        return AsNumber(EvaluateExpr(sub->lhs(), env), "-") -
               AsNumber(EvaluateExpr(sub->rhs(), env), "-");
    }
    if (const auto *mul = dynamic_cast<const gen::Multiply *>(&expr)) {
        return AsNumber(EvaluateExpr(mul->lhs(), env), "*") *
               AsNumber(EvaluateExpr(mul->rhs(), env), "*");
    }
    ...
    if (const auto *eq = dynamic_cast<const gen::Equal *>(&expr)) {
        return ValueEquals(EvaluateExpr(eq->lhs(), env),
                           EvaluateExpr(eq->rhs(), env));
    }
    if (const auto *neq = dynamic_cast<const gen::NotEqual *>(&expr)) {
        return !ValueEquals(EvaluateExpr(neq->lhs(), env),
                            EvaluateExpr(neq->rhs(), env));
    }
    if (const auto *lt = dynamic_cast<const gen::Less *>(&expr)) {
        return ValueLess(EvaluateExpr(lt->lhs(), env),
                         EvaluateExpr(lt->rhs(), env));
    }
    ...
    if (const auto *gt = dynamic_cast<const gen::Greater *>(&expr)) {
        return ValueLess(EvaluateExpr(gt->rhs(), env),
                         EvaluateExpr(gt->lhs(), env));
    }
    ...
    if (const auto *call = dynamic_cast<const gen::Call *>(&expr)) {
        ...
        if (user_it != env.functions.end()) {
            ...
            struct FrameGuard {
                ...
                ~FrameGuard() { env_ref.local_frames.pop_back(); }
            } guard{env};

            const StatementResult result =
                ExecuteStatementList(function.body(), env);
            return result.value;
        }
...
StatementResult ExecuteStatement(const gen::Statement &statement,
    ...
    if (const auto *while_stmt =
        ...
        while (IsTruthy(EvaluateExpr(while_stmt->condition(), env))) {
            const StatementResult body =
                ExecuteStatementList(while_stmt->body(), env);
            if (body.returned) {
                return body;
            }
            last = body.value;
    ...
    if (const auto *for_stmt = dynamic_cast<const gen::ForStmt *>(&statement)) {
        ...
        while (IsTruthy(EvaluateExpr(for_stmt->condition(), env))) {
            const StatementResult body =
                ExecuteStatementList(for_stmt->body(), env);
            if (body.returned) {
                return body;
            }
            last = body.value;
...
void CollectGlobalsFromStatement(const gen::Statement &statement,
                                 SymbolTable &symbols);
