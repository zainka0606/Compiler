...
bool SymbolTable::DeclareClass(std::string name, std::vector<std::string> field_names,
                               std::vector<std::string> method_names) {
    const std::string key = name;
    return classes_
        .emplace(key, ClassInfo{
                          .name = std::move(name),
...
const SymbolTable::ClassInfo* SymbolTable::FindClass(std::string_view name) const {
...
const std::unordered_map<std::string, SymbolTable::ClassInfo>& SymbolTable::Classes() const {
    return classes_;
}
