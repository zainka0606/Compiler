...
class CFGBuilder {
  public:
    ...
    void AddEdge(std::size_t from, std::size_t to, std::string label = {}) {
        graph_.edges.push_back(CFGEdge{.from = from, .to = to, .label = std::move(label)});
    }

    std::vector<std::size_t> BuildStatementList(const std::vector<const gen::Statement*>& statements,
        ...
        for (const gen::Statement* statement : statements) {
            ...
            predecessors = BuildStatement(*statement, std::move(predecessors), function_exit_id);
        }
        return predecessors;
    }
    ...
    CFGGraph BuildGraph(const std::vector<const gen::Statement*>& statements) {
        ...
        std::vector<std::size_t> fallthrough = BuildStatementList(statements, {graph_.entry}, graph_.exit);
  ...
  private:
    std::vector<std::size_t> BuildStatement(const gen::Statement& statement,
                                            std::vector<std::size_t> predecessors,
                                            std::size_t function_exit_id) {
        const std::size_t statement_node = AddNode(DescribeStatementNode(statement));
        ...
        if (const auto* if_stmt = dynamic_cast<const gen::IfStmt*>(&statement)) {
            ...
            std::vector<std::size_t> then_exits =
                BuildStatementList(ToStatementPointers(if_stmt->then_body()), {then_entry}, function_exit_id);
            branch_exits.insert(branch_exits.end(), then_exits.begin(), then_exits.end());
            ...
            if (if_stmt->else_body().empty()) {
                ...
                std::vector<std::size_t> else_exits =
                    BuildStatementList(ToStatementPointers(if_stmt->else_body()), {else_entry}, function_exit_id);
                branch_exits.insert(branch_exits.end(), else_exits.begin(), else_exits.end());
            }
        ...
        if (const auto* while_stmt = dynamic_cast<const gen::WhileStmt*>(&statement)) {
            ...
            const std::vector<std::size_t> body_exits =
                BuildStatementList(ToStatementPointers(while_stmt->body()), {body_entry}, function_exit_id);
        ...
        if (const auto* for_stmt = dynamic_cast<const gen::ForStmt*>(&statement)) {
            const std::size_t init_node = AddNode("for_init " + DescribeForInit(for_stmt->init()));
            AddEdge(statement_node, init_node);

            const std::size_t cond_node = AddNode("for_cond " + DescribeExpr(for_stmt->condition()));
            AddEdge(init_node, cond_node);

            const std::size_t body_entry = AddNode("for_body");
            AddEdge(cond_node, body_entry, "T");
            const std::vector<std::size_t> body_exits =
                BuildStatementList(ToStatementPointers(for_stmt->body()), {body_entry}, function_exit_id);

            if (!body_exits.empty()) {
                const std::size_t update_node = AddNode("for_update " + DescribeForInit(for_stmt->update()));
                for (const std::size_t body_exit : body_exits) {
...
CFGGraph BuildGraphForStatements(std::string name, const std::vector<const gen::Statement*>& statements) {
...
ProgramCFG BuildProgramCFG(const AST& ast) {
    ...
    for (const std::unique_ptr<gen::Item>& item : program.items().items()) {
        ...
        if (const auto* top_statement = dynamic_cast<const gen::TopStatement*>(item.get())) {
    ...
    for (const gen::FunctionDecl* function : functions) {
        out.functions.push_back(
            BuildGraphForStatements("fn " + function->name(), ToStatementPointers(function->body())));
    }
    return out;
}

std::string ProgramCFGToGraphvizDot(const ProgramCFG& cfg, std::string_view graph_name) {
    std::ostringstream out;
    out << "digraph " << compiler::common::SanitizeIdentifier(graph_name, "program_cfg") << " {\n";
    out << "  rankdir=TB;\n";
    out << "  node [shape=box];\n";
    ...
    const auto emit_graph = [&](const CFGGraph& graph) {
        ...
        out << "    label=\"" << compiler::common::EscapeGraphvizLabel(graph.name) << "\";\n";
        out << "    color=gray60;\n";
        for (const CFGNode& node : graph.nodes) {
            out << "    g" << cluster_index << "_n" << node.id
                << " [label=\"" << compiler::common::EscapeGraphvizLabel(node.label) << "\"";
        ...
        for (const CFGEdge& edge : graph.edges) {
            out << "    g" << cluster_index << "_n" << edge.from << " -> g" << cluster_index << "_n" << edge.to;
            if (!edge.label.empty()) {
                out << " [label=\"" << compiler::common::EscapeGraphvizLabel(edge.label) << "\"]";
            }
            out << ";\n";
        }
        out << "  }\n";
