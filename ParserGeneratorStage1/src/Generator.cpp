...
GeneratedParserFiles GenerateCppParser(const GrammarSpecAST& spec,
    ...
    if (spec.grammar_name.empty()) {
        throw BuildException("grammar name is required to generate parser code");
    }
    ...
    const std::string embedded_spec_literal = MakeRawStringLiteral(grammar_spec_source);
...

    {
        ...
        h << "    [[nodiscard]] static const compiler::parsergen1::LR1ParseTable& ParseTable();\n";
...

    {
        ...
        cc << "namespace {\n";
        cc << "constexpr const char* kEmbeddedGrammarSpec = " << embedded_spec_literal << ";\n";
        cc << "} // namespace\n\n";

        cc << "const compiler::parsergen1::LR1ParseTable& " << out.parser_class_name << "::ParseTable() {\n";
        cc << "    static const compiler::parsergen1::LR1ParseTable table = []() {\n";
        cc << "        compiler::parsergen1::LR1ParseTable built = compiler::parsergen1::BuildLR1ParseTableFromGrammarSpec(kEmbeddedGrammarSpec);\n";
        cc << "        if (!built.conflicts.empty()) {\n";
        cc << "            throw compiler::parsergen1::BuildException(\"generated parser grammar has \" + std::to_string(built.conflicts.size()) + \" conflict(s)\");\n";
        cc << "        }\n";
        cc << "        return built;\n";
        cc << "    }();\n";
        cc << "    return table;\n";
        ...
        cc << "    return compiler::parsergen1::ParseTokensToCST(ParseTable(), tokens);\n";
        cc << "}\n\n";

        cc << "std::string " << out.parser_class_name
           << "::CSTToGraphvizDot(const CST& cst, std::string_view graph_name) {\n";
        cc << "    return compiler::parsergen1::CSTToGraphvizDot(cst, graph_name);\n";
        cc << "}\n\n";
