...
std::string UsageText(std::string_view program) {
    ...
    oss << "Outputs (written to a directory derived from the input filename stem):\n";
    ...
    oss << "  ParseTable.dot              LR(1) ACTION/GOTO table (Graphviz)\n\n";
    oss << "Options:\n";
...
std::filesystem::path DeriveOutputDirectory(const std::filesystem::path& input_path) {
    ...
    if (stem.empty()) {
        throw BuildException("failed to derive output directory from input file: " + input_path.string());
    }
...
int RunParserGeneratorCLI(int argc, const char* const* argv) {
    const std::string program = (argc > 0 && argv && argv[0]) ? argv[0] : "ParserGeneratorStage1";
    ...
    try {
        ...
        const GrammarSpecAST spec = compiler::parsergen1::ParseGrammarSpec(source);
        const LR1ParseTable table = compiler::parsergen1::BuildLR1ParseTable(spec);
        const GeneratedParserFiles generated =
            compiler::parsergen1::GenerateCppParser(spec, source, options.header_filename, options.source_filename);
        ...
        WriteTextFile(output_dir / "AST.dot", compiler::parsergen1::GrammarSpecASTToGraphvizDot(spec));
        WriteTextFile(output_dir / "CanonicalCollection.dot",
                      compiler::parsergen1::LR1CanonicalCollectionToGraphvizDot(table.canonical_collection));
        WriteTextFile(output_dir / "ParseTable.dot", compiler::parsergen1::LR1ParseTableToGraphvizDot(table));

        std::cout << "Wrote ParserGeneratorStage1 outputs to " << output_dir.string() << "\n";
        std::cout << "Parser: " << generated.header_filename << ", " << generated.source_filename << "\n";
        std::cout << "States: " << table.canonical_collection.states.size()
                  << ", conflicts: " << table.conflicts.size() << "\n";
        return 0;
    } catch (const ParseException& ex) {
        std::cerr << "parse error at " << ex.line() << ":" << ex.column() << ": " << ex.what() << "\n";
