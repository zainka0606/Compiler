...
LR1ParseTable BuildLR1ParseTableFromGrammarSpec(std::string_view source_text) {
    const GrammarSpecAST spec = compiler::parsergen1::ParseGrammarSpec(source_text);
    return compiler::parsergen1::BuildLR1ParseTable(spec);
}

std::string GrammarSpecASTToGraphvizDot(const GrammarSpecAST& spec, std::string_view graph_name) {
...
std::string LR1CanonicalCollectionToGraphvizDot(const LR1CanonicalCollection& collection, std::string_view graph_name) {
    return compiler::lr1::LR1CanonicalCollectionToGraphvizDot(collection, graph_name);
}
