...
namespace {
using example::slrinput::ExampleInputLexer;
using example::slrinput::ExampleInputTokenKind;
using InputToken = example::slrinput::Token;
...
const compiler::slr::SLRAction *
FindAction(const compiler::slr::SLRParseTable &table, std::size_t state_index,
...
const std::size_t *FindGoto(const compiler::slr::SLRParseTable &table,
...
ParseTree ParseInputWithSLR(const compiler::slr::SLRParseTable &table,
                            const std::vector<InputToken> &tokens) {
    if (!table.conflicts.empty()) {
        throw std::runtime_error("cannot parse with conflicting SLR table (" +
                                 std::to_string(table.conflicts.size()) +
                                 " conflict(s))");
    }
    if (table.canonical_collection.states.empty()) {
        throw std::runtime_error("SLR table has no states");
    }
    ...
    while (true) {
        ...
        const compiler::slr::SLRAction *action =
        ...
        switch (action->kind) {
        case compiler::slr::SLRActionKind::Shift: {
            if (action->target_state == compiler::slr::kInvalidIndex) {
        ...
        case compiler::slr::SLRActionKind::Reduce: {
        ...
        case compiler::slr::SLRActionKind::Accept: {
...
int main(int argc, char **argv) {
    if (argc != 3 && argc != 4) {
        std::cerr << "usage: ExampleSLRParserDriver <grammar-spec> <input> "
                     "[ast-dot-output]\n";
        return 2;
    }
    ...
    try {
        ...
        const compiler::slr::GrammarSpecAST grammar =
            compiler::slr::ParseGrammarSpec(grammar_text);
        const compiler::slr::SLRParseTable table =
            compiler::slr::BuildSLRParseTable(grammar);

        ExampleInputLexer lexer(input_text);
        const std::vector<InputToken> tokens = lexer.Tokenize();

        const ParseTree parse_tree = ParseInputWithSLR(table, tokens);
        WriteTextFile(output_path, ParseTreeToGraphvizDot(parse_tree));
        std::cout << "Wrote AST DOT to " << output_path.string() << "\n";
        return 0;
    } catch (const compiler::slr::ParseException &ex) {
