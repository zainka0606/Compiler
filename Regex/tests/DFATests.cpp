...
void ExpectDFAMatch(std::string_view pattern, std::string_view input) {
    ...
    if (!DFAMatches(dfa, input)) {
        ...
        oss << "expected DFA match: pattern='" << pattern << "', input='"
            << input << "'";
        Fail(oss.str());
    }
}
...
void ExpectDFANoMatch(std::string_view pattern, std::string_view input) {
    ...
    if (DFAMatches(dfa, input)) {
        std::ostringstream oss;
        oss << "expected DFA no match: pattern='" << pattern << "', input='"
            << input << "'";
        Fail(oss.str());
    }
}
...
void ExpectNFADFAParity(std::string_view pattern,
    ...
    for (std::string_view input : inputs) {
        ...
        if (nfa_match != dfa_match) {
            std::ostringstream oss;
            oss << "NFA/DFA mismatch for pattern='" << pattern << "', input='"
                << input << "' (nfa=" << nfa_match << ", dfa=" << dfa_match
                << ")";
            Fail(oss.str());
        }
    }
    ...
    const std::vector<TestCase> tests = {
        {"dfa_structure_is_valid",
         [] {
             ...
             ExpectTrue(dfa.start_state < dfa.states.size(),
