...
class NFABuilder {
  public:
    ...
    [[nodiscard]] NFA Finish(const Fragment root) && {
        NFA nfa;
  ...
  private:
    ...
    void AddTransition(const std::size_t from, NFATransition transition) {
        states_[from].transitions.push_back(std::move(transition));
    }

    void AddEpsilon(const std::size_t from, const std::size_t to) {
    ...
    Fragment BuildLiteral(const char c) {
        const std::size_t start = AddState();
        const std::size_t end = AddState();
    ...
    Fragment BuildCharacterClass(const bool negated,
    ...
    Fragment BuildRepetition(const RegexNode &operand, const std::size_t min,
                             const std::optional<std::size_t> &max) {
        bool has_result = false;
        Fragment result{};

        auto append = [&](const Fragment fragment) {
            if (!has_result) {
                result = fragment;
                has_result = true;
                return;
...
bool TransitionMatches(const NFATransition &transition, const char c) {
...
NFATransition NFATransition::Epsilon(const std::size_t target) {
...
NFATransition NFATransition::Literal(const std::size_t target,
                                     const char literal) {
    NFATransition transition;
...
NFATransition NFATransition::Dot(const std::size_t target) {
    NFATransition transition;
...
NFATransition
NFATransition::CharacterClass(const std::size_t target, const bool negated,
...
NFA CompilePatternToNFA(const std::string_view pattern) {
    return CompileToNFA(Parse(pattern));
}

bool NFAMatches(const NFA &nfa, const std::string_view input) {
    if (nfa.states.empty()) {
        return false;
    }
